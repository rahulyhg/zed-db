...
//            deferred.resolve(ReleaseService.get({id: $route.current.params.id}));

	ReleaseService.get({id: $route.current.params.id}, function(data){
           ...
           /* release: function(ReleaseService, $route) {
                ...
                return ReleaseService.get({id: $route.current.params.id}).$promise;
            }*/
        },
        access: access.user
    ...
    .when('/contacts/:id', {
        ...
        resolve: {
          contact : function(ContactService, $route, $q) {
            var deferred = $q.defer();
            ContactService.get({id: $route.current.params.id}, function(data){
                deferred.resolve(data);
            });
            return deferred.promise;
          }
...
app.factory('AuthenticationService', function ($http, $sanitize, $location, $rootScope, SessionService, FlashService) {
    ...
    var cacheSession   = function(response) {
        SessionService.set('user', JSON.stringify({username: response.username, role: response.role})) 
	...
	return accessLevel.bitMask & role.bitmask; 
        ...
        logout: function() {
          ...
          uncacheSession();
          setUser({ username: '', role: userRoles.public });
