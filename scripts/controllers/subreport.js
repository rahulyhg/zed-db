...
app.controller('SubscriberreportCtrl', function($rootScope, $scope, $http, $timeout, $filter, $location, promiseTracker, SubReportService, SkillsService, SubService, SubtypesService, ProgramsService, PrizesService, limitToFilter) {
    ...
    $scope.gridSubOptions = {
        ...
        columnDefs: [{
                ...
                field: 'posted',
                displayName: 'Posted?',
                cellTemplate: '<div class="ngSelectionCell"><input tabindex="-1" class="ngSelectionCheckbox" type="checkbox" ng-model="row.entity[col.field]" ng-change="updatePosted(col, row)" ng-checked="row.entity[col.field]" /></div>',
                sortable: false,
                width: 50
            }, {
    ...
    $scope.gridPledgeOptions = {
        ...
        columnDefs: [

            {
                ...
                field: 'subscriber.sublastname',
                displayName: 'Last Name'
            }, {
                field: 'subscriber.subfirstname',
                displayName: 'First Name'
            }, {
                field: 'subscriber.subemail',
                displayName: 'Email'
            }, {
                field: 'subscriber.submobile',
                displayName: 'Phone'
            }, {
                field: 'subscriber.subscription.subtypecode',
                displayName: 'Sub Type'
            }, {
                field: 'subscriber.subscription.subtypevalue',
                displayName: 'Sub Value',
                cellFilter: 'currency'
            }, {
                field: 'paymentdate',
                displayName: 'Pledge Payment Date',
                cellFilter: 'date'
            }, {
                field: 'subscriber.paymentdate',
    ...
    $scope.expiresToday = function() {
        ...
        $http.get(apiSrc + '/subscribers/expired/' + today, {
            tracker: 'rTracker'
    ...
    $scope.activeSubs = function() {
        $scope.clearForm();
        $http.get(apiSrc + '/subscriber/types/', {
            tracker: 'rTracker'
        }).then(function(response) {
            $scope.counts = response.data;
            var subtt = 0
            var totals = _.pluck(response.data, 'subtypecount');
            $scope.subsSum = _.reduce(totals, function(memo, num) {
    ...
    $scope.unpaidSubs = function() {
        $scope.clearForm();
        $http.get(apiSrc + '/subscribers/unpaid/', {
            tracker: 'rTracker'
        }).then(function(response) {
            $scope.subs = response.data;
            $scope.emails = $scope.emailList(response.data);
    ...
    $scope.paidToday = function() {
        $scope.clearForm();
        $http.get(apiSrc + '/subscribers/paid/', {
            tracker: 'rTracker'
        }).then(function(response) {
            $scope.subs = response.data;
            $scope.emails = $scope.emailList(response.data);
    ...
    $scope.pledgeOut = function() {
        $scope.clearForm();
        $http.get(apiSrc + '/subscribers/pledgenew/', {
            tracker: 'rTracker'
        }).then(function(response) {
            ...
            angular.forEach(wsub, function(d) {
                if (d.subscriber.subemail != null && d.subscriber.subemail != '') {
                    text = text + d.subscriber.subemail + ', ';
                }
            });
    ...
    $scope.unPosted = function() {
        $scope.clearForm();
        $http.get(apiSrc + '/subscribers/notposted/', {
            tracker: 'rTracker'
        }).then(function(response) {
            $scope.subs = response.data;
            $scope.emails = $scope.emailList(response.data);
    ...
    $scope.getEmailList = function() {
        $scope.clearForm();
        $http.get(apiSrc + '/subscriber/emaillist/', {
            tracker: 'rTracker'
        }).then(function(response) {
    ...
    $scope.updatePosted = function(col, row) {
        $scope.sub = SubService.get({
            id: row.entity.subnumber
        });
        if (row.entity.posted === false) {
            row.entity.posted = 0;
        }
    ...
    $scope.search = function() {
        ...
        if ($scope.subscriberSearchForm.$dirty === true) {
            ...
            $http.post(apiSrc + '/subscriber/report/', $scope.subscriberSearchFormData, {
                tracker: 'rTracker',
                headers: {
    ...
    $scope.getSubsByPrize = function($radiothonprizeid) {
        $http.get(apiSrc + '/subscribers/reports/pledge/' + $radiothonprizeid, {
            tracker: 'rTracker'
        }).then(function(response) {
