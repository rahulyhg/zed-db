...
app.controller('ContactmgmtCtrl', function($rootScope, $scope, $http, $location, DepartmentsService, InterestsService, limitToFilter, $dialog) {
    ...
    $scope.gridCategoryOptions = {
        ...
        columnDefs: [{
            ...
            enableCellEdit: true,
            editableCellTemplate: catCell
        }, {
            field: '',
            cellTemplate: '<button class="close" style="float:left;padding-left:5px;" ng-click="deleteCategory(col, row);">&times;</button> ',
            width: 30
        }]
    };
    ...
    $scope.gridSubcategoryOptions = {
        ...
        columnDefs: [{
            ...
            enableCellEdit: true,
            editableCellTemplate: subCatCell
        }, {
            field: '',
            cellTemplate: '<button class="close" style="float:left;padding-left:5px;" ng-click="deleteSubcategory(col, row);">&times;</button> ',
            width: 30
        }]
    };
    ...
    $scope.updateCategories = function(col, row) {
        $scope.category = DepartmentsService.get({
            id: row.entity.department_no
        });
    ...
    $scope.updateSubcategories = function(col, row) {
        $scope.subcategory = InterestsService.get({
            id: row.entity.interest_no
        });
        $scope.subcategory.interest_nm = row.entity.interest_nm;
        $scope.subcategory.$update({
            id: row.entity.interest_no
        });
