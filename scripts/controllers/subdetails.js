...
app.controller('SubdetailsCtrl', function ($rootScope, $scope, $http, $timeout, $routeParams, SubService, promiseTracker, SkillsService, ProgramsService, PledgeService, PrizesService, SubtypesService, $dialog, $location, limitToFilter, $anchorScroll, $filter) {
        ...
        if ($scope.subForm.$valid === true) {
            if ($routeParams.id) {
                ...
                $http.get(apiSrc + '/subscribers/lastrec/').success(function (data) {
                    ...
                    var nlog = _.sortBy(log, function(num){ return num; });
    ...
    $scope.addBm = function () {
        var newbm = {subbandmemberfirstname: $scope.t.fname, subbandmemberlastname: $scope.t.lname, subid: $scope.sub.subnumber};
    ...
    $scope.generateReceiptS = function () {
        ...
        }).error(function() { console.log('no luck bud.')});
    };
