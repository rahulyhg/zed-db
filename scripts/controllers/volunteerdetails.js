...
app.controller('VolunteerDetailsCtrl', function($rootScope, $scope, $http, $timeout, $stateParams, VolunteerService, VolDepartmentsService, SkillsNewService, SkillsService, QualificationService, TrainingService, $dialog, $location, limitToFilter, $anchorScroll, $filter, volunteer) {
    ...
    if ($scope.volunteer.volunteer.completed_orientation_date) $scope.volunteer.volunteer.completed_orientation_date = new Date($scope.volunteer.volunteer.completed_orientation_date);

	$scope.qualifications = QualificationService.query(function() {
		if ($scope.volunteer.qualifications) {
			// pluck out id from json and create new array
			var fakeQ = function() {
	...
	$scope.training = TrainingService.query(function() {
		if ($scope.volunteer.training) {
			var fakeQ = function() {
				$scope.trArr = _.pluck($scope.volunteer.training, 'id');
			};
			$timeout(fakeQ, 300);
		}
	});
	...
	$scope.saveVolunteer = function() {
		...
		if ($scope.volunteerForm.$valid === true) {
				if ($stateParams.id) {
					...
					$scope.volunteer.training = $scope.trArr;
